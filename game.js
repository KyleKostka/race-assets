...
try{window.__GOL&&(window.__GOL.CAM=CAM,window.__GOL.setCam=setCam,window.__GOL.FC=FC)}catch(q){}
...
for(let it=0;it<trials;it++){let rr=$((p.seed^(it*2654435761))>>>0),sc=[];
for(let c=0;c<S;c++){let a=b[c],cp=cps[a.comp],age=0,tsum=0;
for(let L=0;L<laps;L++){let wear=Math.min(.14,cp.w*Math.pow(age,1.3)+(age>cp.cliff?.012*(age-cp.cliff):0));
tsum+=1/(a.rating*cp.m*(1-wear));age++}
let score=-tsum*100-a.gridPos*.16-a.stops.length*.55+(rr()-.5)*32;
if(rr()<.055)score-=999;
sc.push({c:c,s:score})}
sc.sort((x,y)=>y.s-x.s);
...
async function startFromMenu(){
  ...
  try{
    var r=await v({track:MENU.track,laps:MENU.laps});
    if(r===null){showMenu()}
  }catch(err){
...
g.globalAlpha=1}
try{window.__GOL={get p(){return p},get b(){return b},get M(){return M},get C(){return C},CAM:null,setCam:null}}catch(q){}
